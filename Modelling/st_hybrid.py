...
normalized_folder1 = os.path.join('..', 'normalized/hybrid/train')
normalized_folder2 = os.path.join('..', 'normalized/hybrid/test')

data = pd.read_csv(os.path.join(normalized_folder1, 'sn_new_hybrid_nor.csv'))

# Load test datasets
test_datasets = {
    'c': pd.read_csv(os.path.join(normalized_folder2, 'sn_hybrid_c_nor.csv')),
    'm': pd.read_csv(os.path.join(normalized_folder2, 'sn_hybrid_m_nor.csv')),
    'i': pd.read_csv(os.path.join(normalized_folder2, 'sn_hybrid_i_nor.csv')),
    'n': pd.read_csv(os.path.join(normalized_folder2, 'sn_hybrid_n_nor.csv')),
    'lp': pd.read_csv(os.path.join(normalized_folder2, 'lp_hybrid_nor.csv')),
    'rea': pd.read_csv(os.path.join(normalized_folder2, 'rea_hybrid_nor.csv')),
    'ytb': pd.read_csv(os.path.join(normalized_folder2, 'ytb_hybrid_nor.csv')),
    'mg': pd.read_csv(os.path.join(normalized_folder2, 'mg_hybrid_nor.csv'))
}

# Feature selection
features = ['cache-misses', 'context-switches','page-faults','minor-faults','major-faults','branch-misses','instructions', 'cpu-cycles','cache-references','msr/tsc/','cpu-clock','ex_ret_instr','seconds user','seconds sys','accept', 'accept4', 'access', 'alarm', 'arch_prctl', 'bind', 'brk', 'clock_gettime', 'clock_nanosleep', 'clone', 'close', 'connect', 'execve', 'fcntl', 'flock', 'fsync', 'ftruncate', 'futex', 'getcwd', 'getdents64', 'geteuid', 'getpeername', 'getpid', 'getppid', 'getrandom', 'getrusage', 'getsockname', 'getsockopt', 'gettimeofday', 'getuid', 'ioctl', 'kill', 'listen', 'lseek', 'madvise', 'mkdir', 'mlock', 'mmap', 'mprotect', 'msync', 'munmap', 'newfstatat', 'openat', 'perf_event_open', 'pipe2', 'prctl', 'pread64', 'preadv', 'preadv2', 'prlimit64', 'pselect6', 'pwritev', 'pwritev2', 'read', 'readv', 'recvfrom', 'rmdir', 'rseq', 'rt_sigaction', 'rt_sigprocmask', 'sched_getaffinity', 'sendto', 'set_robust_list', 'set_tid_address', 'setpgid', 'setsockopt', 'shutdown', 'sigaltstack', 'socket', 'statfs', 'sysinfo', 'times', 'truncate', 'umask', 'uname', 'unlink', 'wait4', 'write', 'writev']
target = 'energy'
