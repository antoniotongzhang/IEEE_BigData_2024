...
for val2 in "${array2[@]}"; do
	for val3 in "${array3[@]}"; do
		...
		for val4 in "${array4[@]}"; do
                ...
                val4_split=($val4)
                combination="${val2}${val3_split[0]}${val3_split[1]}${val3_split[2]}${val4_split[0]}${val4_split[1]}${val4_split[2]}"
...
do
	for combination in "${shuffled_combinations[@]}"; do 
		...
		var6="${combination:20:4}"
		if [ $combination_length -gt 37 ]; then
			var7="${combination:24:14}"
			var8="${combination:38}"
			stress-ng --sock 1 --sock-domain $var6 $var7 --sock-opts $var8 --vm 1 --vm-bytes $var2 --hdd 1 --hdd-bytes $var5 --hdd-opts fsync,$var3,$var4 -t 15
		else
			var7="${combination:24}"
			stress-ng --sock 1 --sock-domain $var6 --sock-opts $var7 --vm 1 --vm-bytes $var2 --hdd 1 --hdd-bytes $var5 --hdd-opts fsync,$var3,$var4 -t 15
		fi
	done
done
